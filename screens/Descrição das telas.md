...
## Conceito:
*Totais anuais de quantidade de procedimentos registrados e valores financeiros*

## Descrição/elementos:
### Cards de KPI
- **Tipo de elemento:** Card (indicador único).

- **Tipo de informação:** Métricas agregadas de produção e faturamento ambulatorial.

- **Tipos de filtros:** Dependem dos filtros globais aplicados, como período (ano).

### Gráfico de Barras Horizontais – Produção por Subgrupo de Procedimento
- **Tipo de elemento:** Bar chart horizontal.

- **Tipo de informação:** Quantidade de procedimentos agrupados por subgrupo (ex.: clínicos, cirúrgicos).

Tipos de filtros: Filtros globais de período e segmentações adicionais.

### Gráfico de Linha – Faturamento Ambulatorial
- **Tipo de elemento:** Line chart com múltiplas séries.

- **Tipo de informação:** Evolução temporal do faturamento, segmentado por status (ex.: aprovado, não aprovado).

Tipos de filtros: Segmentação por ano e filtros globais.

### Gráfico de Rosca – Atendimentos por Gênero
- **Tipo de elemento:** Donut chart.

- **Tipo de informação:** Distribuição percentual e absoluta dos atendimentos por categoria de gênero.

Tipos de filtros: Segmentação por ano e botão de ação para alternar comportamento relacionado a BPA.

### Gráfico de Barras Horizontais – Origem do Cidadão
- **Tipo de elemento:** Bar chart horizontal.

- **Tipo de informação:** Distribuição de atendimentos por local de origem (UF ou não informado).

Tipos de filtros: Filtros globais de período e segmentações.

### Gráfico de Barras Horizontais – Atendimentos por CBO
- **Tipo de elemento:** Bar chart horizontal.

- **Tipo de informação:** Quantidade de atendimentos agrupados por ocupação profissional (CBO).

- **Tipos de filtros:** Filtros globais.

### Gráfico de Barras Horizontais – Faturamento por CBO
- **Tipo de elemento:** Bar chart horizontal.

- **Tipo de informação:** Valor de faturamento agrupado por ocupação profissional (CBO).

Tipos de filtros: Filtros globais.

### Botão de Ação
- **Tipo de elemento:** Botão interativo.

- **Tipo de informação:** Alterna inclusão ou exclusão de BPA Consolidado (registros sem informação de sexo).

Tipos de filtros: Atua sobre o comportamento dos visuais relacionados a BPA.

### Segmentador de Período
- **Tipo de elemento:** Slicer horizontal (botões).

- **Tipo de informação:** Seleção de períodos temporais discretos (anos).

Tipos de filtros: Filtro global que afeta todos os visuais do dashboard.

### Interações Implícitas
- **Tipo de elemento:** Cross-filter / Cross-highlight.

- **Tipo de informação:** Relações de filtragem e destaque entre visuais.
